...
function buildMatchedRowFromScrapedModel(
    ...
    const artificialAnalysisModelId =
        typeof scrapedModel.model_id === "string"
    ...
    const intelligenceIndexCost = asRecord(
        scrapedModel.intelligence_index_cost,
    );
    const logo =
    ...
    const apiIntelligenceIndexCost = asRecord(
        apiModel?.intelligence_index_cost,
    );
...
export async function buildMatchedRows(
    ...
    return fallbackDiagnostics.models
        .map((matchedModel) => {
            ...
            if (
                typeof matchedModelId !== "string" ||
                matchedModelId.length === 0
            ) {
                return null;
            }
            if (
