...
function candidateScaleValue(
    ...
    const nameValue = firstParsedNumber(
...
function sameVariantReward(
    ...
    if (
        sourceLastToken === baseLastToken ||
        sourceLastToken === nameLastToken
    ) {
...
export function scoreCandidate(
    ...
    if (
        hasHardBScaleMismatch(sourceSlug, candidateModelId, candidateModelName)
    ) {
        return 0;
    }
