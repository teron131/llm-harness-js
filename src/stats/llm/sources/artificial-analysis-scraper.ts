    ...
    return rows.map((row) =>
        Object.fromEntries(
            Object.entries(row).filter(
                ([column]) => !columnsToDrop.has(column),
            ),
        ),
    );
}
...
function getSelectedColumnValue(
    ...
    switch (column) {
        ...
        case "release_date":
            return typeof row.release_date === "string"
                ? row.release_date
                : null;
        case "input_tokens": {
            const intelligenceTokenCounts = asRecord(
    ...
    return rows.map((row) => {
        ...
        for (const column of keepSet) {
            ...
            if (columnValue !== NO_COLUMN_VALUE) {
                ...
                selectedRow[column] = normalizeUndefinedToNull(
                    row[column] ?? null,
                );
            }
        }
...
function extractRowsFromCorpus(corpus: string): JsonObject[] {
    ...
    while (true) {
        ...
        const searchStart = Math.max(
            0,
            hitIndex - MODEL_SEARCH_BACKTRACK_CHARS,
        );
        for (
            let backIndex = hitIndex;
            backIndex >= searchStart;
            backIndex -= 1
        ) {
            if (corpus[backIndex] !== "{") {
                continue;
...
export async function getArtificialAnalysisScrapedRawStats(
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Artificial Analysis scrape failed: ${response.status}`,
            );
        }
