    ...
    for (let attempt = 0; attempt < maxRetries; attempt += 1) {
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    `OpenRouter request failed: ${status} (${url})`,
                );
            }
...
function buildSlugFallbackCandidates(
    ...
    const scoredCandidates = availableSlugs
        .filter(
            (slug) => slug.startsWith(`${provider}/`) && slug !== normalized,
        )
        .map((slug) => {
...
async function fetchPerformanceForPermaslug(
    ...
    const [throughput, latency, latencyE2e, effectivePricing] =
        await Promise.all([
...
export async function getOpenRouterScrapedStats(
    ...
    const uniqueModelIds = Array.from(
        new Set(
            options.modelIds.map((modelId) => modelId.trim()).filter(Boolean),
        ),
    );
